...
def plot_images(generator, noise_input, noise_class, show=False, step=0, name="conditional-gan"):
    '''
    DCGAN 과는 다르게 'noise_class' 가 추가된 형태
    Conditional GAN 은 class 에 대한 one-hot encoding 을 같이 주게 되어 조건부 확률 분포를 생성하고자 함
    '''

    os.makedirs(name, exist_ok=True)
    filename = os.path.join(name, "%05d.png" % step)

    images = generator.predict([noise_input, noise_class]) # DCGAN 과 다르게 generator 은 noise_class 까지 받아서 생성하게 된다
...
def build_generator(inputs, y_labels, img_size):
    '''
    DCGAN 과 다르게 모델의 입력으로 input 과 y_label 을 같이 받는다
    '''
    img_resize = img_size // 4

    x = concatenate([inputs, y_labels], axis=1) # input 과 y_labels 를 concatenate 하여 입력값을 구성함, if input : 64 x 100 and y_labels : 64 x 10, results : 64 x 110
...
def train_models(models, data, **kwargs):
    (batch_size, latent_size, steps, img_save_interval, name) = 64, 100, 40000, 2000, 'conditional-gan'  # Default
    ...
    # Initialization
    img_save_noise_vectors = np.random.uniform(-1.0, 1.0, size=[16, latent_size])  
    img_save_noise_class = np.eye(10)[np.arange(0, 16) % 10] # noise vector 과 동일하게 16개의 one-hot encoding vector 생성, 각 vector의 차원은 10
    ...
    for i in range(steps):
        ...
        noise_vectors = np.random.uniform(-1.0, 1.0, size=[batch_size, latent_size]) # 64 x 100 array
        fake_labels = np.eye(10)[np.random.choice(10, batch_size)] # 64 x 10 array, 64개의 one-hot encodings
        fake_imgs = generator.predict([noise_vectors, fake_labels])

        x = np.concatenate([real_imgs, fake_imgs]) # Discriminator 의 input
        y = np.concatenate([np.ones([batch_size]), np.zeros([batch_size])]).reshape(-1, 1) # Real / Fake labeling
        ...
        y = np.ones([batch_size, 1]) # 64 x 1 형태의 1로 구성된 array
...
#%%
    ...
    train_models(models, (x_train, y_train), batch_size=64, latent_size=z_size, steps=20000, img_save_interval=2000, name=model_name)
# %%
